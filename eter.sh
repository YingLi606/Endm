...
validity_git() {
	...
	if [ "${git}" = "" ]; then
		...
		case ${wheregit} in
		1)
			Modify_the_variable git "https:\/\/github.com\/" ${HOME}/EterUl/config/config.sh
			Modify_the_variable rawgit "https:\/\/raw.githubusercontent.com\/EterUltimate\/EterUl\/refs\/heads\/main\/" ${HOME}/EterUl/config/config.sh
			return 0
			;;
		2)
			Modify_the_variable git "https:\/\/gh.xmly.dev\/https:\/\/github.com\/" ${HOME}/EterUl/config/config.sh
			Modify_the_variable rawgit "https:\/\/gh.xmly.dev\/https:\/\/raw.githubusercontent.com\/EterUltimate\/EterUl\/refs\/heads\/main\/" ${HOME}/EterUl/config/config.sh
			return 0
			;;
		*)
...
validity_auto_upgrade() {
	...
	if [ "${auto_upgrade}" = "" ]; then
		...
		case ${wheregit} in
		1)
			Modify_the_variable auto_upgrade "true" ${HOME}/EterUl/config/config.sh
			log "自动升级脚本开启"
			return 0
			;;
		2)
			Modify_the_variable auto_upgrade "false" ${HOME}/EterUl/config/config.sh
			log "自动升级脚本关闭"
			return 0
			;;
		*)
...
validity_dir() {
	mkdir -p ${HOME}/EterUl/{download,config}
	mkdir -p ${HOME}/.back
	mkdir -p ${HOME}/.TEMP
}
...
apt_up() {
	...
	if [[ -z "${last_time_aptup}" || $((current_timestamp - last_time_aptup)) -ge $((5 * 24 * 60 * 60)) ]]; then
		if [ "${auto_upgrade}" = "true" ]; then
			...
			Modify_the_variable last_time_aptup ${current_timestamp} ${HOME}/EterUl/config/config.sh
		else
			log "自动升级脚本未开启"
		fi
	fi
...
debuger() {
	echo "${INFO}脚本定义的变量：$(cat ${HOME}/EterUl/config/config.sh)" 
	case $(uname -o) in
	Android)
...
-s | --start)
	...
	Android | A)
		log "指定加载安卓功能"
		source ${HOME}/EterUl/local/Android/Android_menu $3 $4 $5
		;;
	Linux | L)
		log "指定加载Linux功能"
		source ${HOME}/EterUl/local/Linux/Linux_menu $3 $4 $5
		;;
	esac
	;;
*)
	...
	Android)
		...
		validity
		variable
		bash ${HOME}/EterUl/function/update.sh
		log "检查更新"
		source ${HOME}/EterUl/local/Android/Android_menu $1 $2 $3
		sleep 2
		clear
		;;
	*)
		...
		bash ${HOME}/EterUl/function/update.sh
		log "检查更新"
		source ${HOME}/EterUl/local/Linux/Linux_menu $1 $2 $3
		sleep 2
		clear
		;;
	esac
